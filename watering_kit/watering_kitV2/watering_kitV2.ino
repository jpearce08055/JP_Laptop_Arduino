...
// Set OK to water flag - water only 10 seconds per day
int OK_to_water = 1;
uint32_t PumpTime = 0L; // long integer 
uint32_t NewTime = 0L; // Cum millis time at this check of pump status
uint32_t LastTime = 0L; // Cum millis time at the last check of pump status
DateTime dt; //Set a datetime with current TRC data and time 
int LastDay = 0; // The day the last time the pump status was checked
...
//--------------------------------------------------------------------
void CheckPumpTime () 
// Check to see if the pump has been running for more than 5 seconds in a day
// If so disable it until the next day
{
  DateTime nowTime = RTC.now;
  
  NewTime = millis();
  // millis has rolled over and started at zero - skip the check
  if (NewTime < LastTime) NewTime = LastTime; 
  
  if (pump_state_flag == 1)
    PumpTime += NewTime - LastTime ;
    
  if (PumpTime > 5000L) // Pump has been on for more than 5 seconds
    OK_to_water = 0; // Stop watering for that day

  if (nowTime.day() > LastDay) //Day has changed - can water another 5 seconds on this day
    {
      OK_to_water = 1; 
      PumpTime = 0L;   
    }

  NewTime = LastTime; // update cum time counter
  LastDay = nowTime.day(); // update to current day
}
//---------------------------------------------------------------------
